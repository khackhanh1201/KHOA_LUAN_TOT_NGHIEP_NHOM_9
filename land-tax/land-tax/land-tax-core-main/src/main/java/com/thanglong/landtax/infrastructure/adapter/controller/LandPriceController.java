...
import com.thanglong.landtax.usecase.service.LandPriceExportService;
...
public class LandPriceController {
    ...
    private final LandPriceExportService landPriceExportService;
    ...
    static class LandPriceRequest {
        ...
        /** ID tài liệu quyết định đã upload qua POST /api/files/upload */
        private Long decisionDocumentId;
    }
    ...
    /** Xuất bảng giá đất ra file Excel (.xlsx). Có thể lọc theo từ khóa (giống ô tìm kiếm FE). */
    @GetMapping("/export")
    @PreAuthorize("hasAnyRole('ADMIN', 'LAND_OFFICER')")
    @com.thanglong.landtax.infrastructure.config.aop.AuditLog(action = "Xuất Excel bảng giá đất")
    public ResponseEntity<byte[]> exportPrices(@RequestParam(required = false) String keyword) {
        log.info("GET /api/land-prices/export - keyword={}", keyword);
        try {
            byte[] excelBytes = landPriceExportService.exportToExcel(keyword);
            String dateStr = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
            String filename = "bang-gia-dat-" + dateStr + ".xlsx";
            ...
            log.error("Export land prices failed: {}", e.getMessage(), e);
    ...
    /** Tài liệu quyết định đính kèm bảng giá đất. */
    @GetMapping("/{id}/documents")
    @PreAuthorize("hasAnyRole('ADMIN', 'LAND_OFFICER', 'TAX_OFFICER', 'CITIZEN')")
    public ResponseEntity<List<DocumentSummaryDTO>> getPriceDocuments(@PathVariable Integer id) {
        log.info("GET /api/land-prices/{}/documents", id);
        if (!landPriceJpaRepository.existsById(id)) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(recordDocumentService.toSummaries(
                recordDocumentService.findLandPriceDocuments(id)));
    }
    ...
    public ResponseEntity<?> createPrice(@Valid @RequestBody LandPriceRequest req) {
        ...
        if (req.getDecisionDocumentId() != null) {
            recordDocumentService.linkLandPriceDocument(saved.getPriceId(), req.getDecisionDocumentId());
        }
    ...
    public ResponseEntity<?> updatePrice(@PathVariable Integer id,
        ...
        return landPriceJpaRepository.findById(id)
                .<ResponseEntity<?>>map(existing -> {
                    ...
                    if (req.getDecisionDocumentId() != null) {
                        recordDocumentService.linkLandPriceDocument(updated.getPriceId(), req.getDecisionDocumentId());
                    }
                    return ResponseEntity.ok(Map.of(
                            "data", updated,
                            "message", "Cập nhật bảng giá đất thành công"));
