...
public class TaxExemptService {
    ...
    public TaxExemptSubjectResponseDTO submitExemption(TaxExemptRequestDTO request, String cccd) {
        ...
        var account = accountLookupService.requireCitizenAccount(citizen.getCitizenId());
    ...
    public TaxExemptSubjectResponseDTO approveExemption(Integer id, String status) {
        ...
        var officerAccount = accountLookupService.requireProcessorAccount(officerCitizen.getCitizenId());
        ...
        if (saved.getCitizenId() != null && saved.getAppliedYear() != null
                && ("APPROVED".equals(status) || "REJECTED".equals(status))) {
            taxPaymentAmountService.refreshPaymentsForCitizenYear(
                    saved.getCitizenId(), saved.getAppliedYear());
        }
    ...
    private TaxExemptSubjectResponseDTO mapToResponseDTO(TaxExemptSubjectEntity entity) {
        ...
        String cccdNumber = entity.getCitizenId() != null
                ? citizenLocalJpaRepository.findById(entity.getCitizenId())
                        .map(c -> c.getCccdNumber())
                        .orElse(null)
                : null;
