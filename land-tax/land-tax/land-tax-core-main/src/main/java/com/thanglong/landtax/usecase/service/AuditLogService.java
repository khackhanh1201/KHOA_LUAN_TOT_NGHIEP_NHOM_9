...
public class AuditLogService {
    ...
    private AuditLogResponseDTO toDto(ProcessingLogEntity entity) {
        String cccd = null;
        String fullName = null;
    
        if (entity.getProcessorAccountId() != null) {
            var accountOpt = accountJpaRepository.findById(entity.getProcessorAccountId());
            if (accountOpt.isPresent()) {
                var citizenOpt = citizenLocalJpaRepository.findById(accountOpt.get().getCitizenId());
                if (citizenOpt.isPresent()) {
                    cccd = citizenOpt.get().getCccdNumber();
                    fullName = citizenOpt.get().getFullName();
                }
            }
        }
