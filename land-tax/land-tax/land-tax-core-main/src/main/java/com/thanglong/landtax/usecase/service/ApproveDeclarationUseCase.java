...
public class ApproveDeclarationUseCase {
    ...
    public Map<String, Object> approveDeclaration(Integer recordId, ReviewDeclarationRequest request) {
        ...
        if (!"PROCESSING".equals(oldStatus)) {
            throw new RuntimeException(
                    "Chỉ có thể phê duyệt hồ sơ đã được cán bộ thuế tiếp nhận (PROCESSING). " +
        ...
        // ===== BUOC 3b: Khai bao dat moi — gan chu so huu khi duyet =====
        if (RecordCategories.isLandOwnershipNew(record.getRecordCategory())) {
            landParcelService.assignPrimaryOwner(record.getCitizenId(), record.getLandParcelId());
            auditLogService.log("ASSIGN_LAND_OWNER", "LAND_OWNERSHIP_NEW",
                    String.valueOf(record.getLandParcelId()),
                    "Assigned citizen " + record.getCitizenId() + " as PRIMARY owner on approve of record "
                            + recordId);
        }
        ...
        String message = RecordCategories.isLandOwnershipNew(record.getRecordCategory())
                ? "Da duyet ho so va gan chu so huu cho thua dat"
                : "Declaration #" + recordId + " has been approved successfully";
