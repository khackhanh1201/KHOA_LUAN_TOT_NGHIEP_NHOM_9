...
export async function exportTaxProcessingRecord({ selectedRecord, exportFormat, dispatch, showAlert }) {
    if (!selectedRecord) return;

    const exportedRecordCode = formatTaxRecordCode(
      selectedRecord.id,
    ...
    if (exportFormat === 'DOC') {
      ...
      fileContent = `
        ...
        <body>
          ...
          <p style='text-align: center;'>Mã hồ sơ: ${exportedRecordCode}</p>
          
          <div class='section'>I. THÔNG TIN NGƯỜI NỘP</div>
          <table class='table'>
            <tr><td style='width: 150px;'>Họ và tên:</td><td><b>${selectedRecord.name || '—'}</b></td></tr>
            <tr><td>Số CCCD/CMND:</td><td><b>${selectedRecord.cccd || '—'}</b></td></tr>
            <tr><td>Địa chỉ thường trú:</td><td>${selectedRecord.address || '—'}</td></tr>
          </table>

          <div class='section'>II. THÔNG TIN THỬA ĐẤT VÀ NGHĨA VỤ THUẾ</div>
          <table class='table'>
            <tr><td style='width: 150px;'>Thửa đất số:</td><td><b>${selectedRecord.parcelNumber || '—'}</b></td></tr>
            <tr><td>Tờ bản đồ số:</td><td><b>${selectedRecord.mapSheetNumber || '—'}</b></td></tr>
            <tr><td>Diện tích thửa đất:</td><td><b>${selectedRecord.areaSize || '—'}</b></td></tr>
            <tr><td>Loại đất sử dụng:</td><td><b>${selectedRecord.landType || '—'}</b></td></tr>
            <tr><td>Địa chỉ thửa đất:</td><td>${selectedRecord.address || '—'}</td></tr>
          </table>

          <div class='section'>III. CHI TIẾT NGHĨA VỤ TÀI CHÍNH</div>
          <table class='table'>
            <tr><td style='width: 150px;'>Loại thuế/lệ phí:</td><td><b>Thuế sử dụng đất phi nông nghiệp</b></td></tr>
          </table>
          <div class='amount-box'>
            <p style='margin: 0; font-size: 12px;'>TỔNG SỐ TIỀN PHẢI NỘP</p>
            <h2 style='margin: 8px 0 0; font-size: 22px;'>${selectedRecord.totalTax || '1.200.000đ'}</h2>
          </div>
      ...
      fileContent = `
        ...
        <body onload="window.print()">
          ...
          <div class="subtitle">Mã hồ sơ: ${exportedRecordCode}</div>

          <div class="section-header">THÔNG TIN NGƯỜI NỘP</div>
          <div class="info-grid">
            <div class="label">Họ và tên:</div>
            <div class="value">${selectedRecord.name || '—'}</div>
            <div class="label">Số CCCD/CMND:</div>
            <div class="value">${selectedRecord.cccd || '—'}</div>
          ...
          <div class="info-grid">
            ...
            <div class="value">Thuế sử dụng đất phi nông nghiệp</div>
          </div>

          <div class="amount-card">
            <div class="amount-title">Tổng số tiền phải nộp</div>
            <div class="amount-value">${selectedRecord.totalTax || '1.200.000đ'}</div>
          </div>
    ...
    if (exportFormat === 'PDF') {
      try {
        await exportHtmlStringToPdf(
          fileContent,
          `thong_bao_thue_${exportedRecordCode}.pdf`
        );
    ...
    link.href = url;
    link.setAttribute('download', `thong_bao_thue_${exportedRecordCode}.${fileExtension}`);
